...
typedef enum PL_PARAM_ATTRIBUTES
{
    ...
    */
    ...
    If the param_id can be either written to or read from the next step
...
typedef enum PL_CLEAR_MODES
{
    ...
    */
    ...
    the sequence had one exposure. The result is the same as using
    #CLEAR_PRE_EXPOSURE.
    */
    CLEAR_PRE_SEQUENCE,
    /**
...
typedef enum PL_FRAME_DELIVERY_MODES
{
    ...
    */
    ...
    in the next interval (unlike the FPS mode where the sensor would be sampled immediately
...
/**
Used with the #pl_exp_abort function. These modes are applicable for CCD
cameras only. For latest sCMOS cameras, please use the #CCS_HALT mode.
...
typedef enum PL_CCS_ABORT_MODES
{
    ...
    */
    ...
    Recommended for sCMOS cameras and for most acquisitions with CCD
...
typedef enum PL_CIRC_MODES
{
    ...
    */
    ...
    replaced by new frame. No error is indicated by any function.
    */
    CIRC_OVERWRITE,
    /**
    In non-overwrite mode the oldest frame in circular buffer is also replaced
    by new frame as in #CIRC_OVERWRITE mode (e.g. via to DMA from camera HW),
    but PVCAM recognizes that and returns an error about buffer overrun in next
...
The resolution defines in which units is the exposure passed to #pl_exp_setup_seq and
...
typedef struct md_frame_roi
{
    ...
    uns16                   extMdDataSize;  /**< Size of the ext. metadata buffer. */
}
md_frame_roi;
...
/******************************************************************************/
/**
Following macros can be used to extract a single byte from multi-byte integers,
...
internal triggered (timed) mode and set up for continuous (circular buffer)
acquisition.
...
The serial number for Photometrics-branded cameras has a maximum length of
...
@warning Be very careful while this parameter. It is known to hang some
...
/**
@brief Pixel sample range in number of bits for the currently configured acquisition.
...
Most Photometrics cameras transfer pixels in 16-bit words (#uns16 type). However, some cameras are
capable of transferring 8-bit data (#uns8 type). Make sure to also check the #PARAM_IMAGE_FORMAT
parameter to discover what is the current camera pixel format.
...
automatically if needed. This applies for the metadata reported ROI and color mask.
...
also reported flipped: an RGGB sensor mask would be reported as GRBG mask.

If #PARAM_METADATA_ENABLED is enabled the frame-embedded metadata is also adjusted
automatically if needed. This applies for the metadata reported color mask value.
...
It returns the time for each pixel, in nanoseconds. This value may
...
set to max. 32-bit range for backward compatibility. It means the range is not
...
It applies to currently configured acquisition. This parameter becomes
...
with @c TRUE value to rest the timestamp.
...
the camera name is reported, but the camera will not be available. #pl_cam_get_name returns a name,
...
parameters have not changed meanwhile, the PVCAM will not resend the configuration. This may
...
/**
@brief Decodes a metadata-enabled frame data into provided frame descriptor structure.

This function processes the input frame and calculates pointers to frame metadata header,
ROI header(s) and ROI image data and stores those into the provided @a pDstFrame structure.
...
@param[out] pDstFrame   A pre-allocated helper structure that will be filled with
                        information based on the input frame buffer.
